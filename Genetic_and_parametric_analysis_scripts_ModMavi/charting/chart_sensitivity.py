...
Module description
------------------
Creates sensitivity regression charting from any number of single parameter numeric
sensitivity analyses generated from parametric_sensitivity.py. Both the x & y axes are
standardized so this shows the sensitivity the parameter has on the target metric.
...
def sensitivity(df: pd.DataFrame, target):
     ...
     title = 'Parameter sensitivity (' + target + ')'
     fig = px.bar(  df,
                    y='beta',

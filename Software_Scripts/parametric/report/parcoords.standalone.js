...
(function (global, factory) {
    ...
    var updateAxes = function updateAxes(config, pc, position, axis, flags) {
      return function () {
        ...
        }).append('svg:text').style("text-anchor", "start").style("transform", "rotate(340deg)").attr('class', 'label').attr('x', 0).attr('y', 0).attr('transform', 'translate(0,-5) rotate(' + config.dimensionTitleRotation + ')').text(dimensionLabels(config)).on('wheel', rotateLabels(config, pc));
    ...
    var createAxes = function createAxes(config, pc, xscale, flags, axis) {
      return function () {
        ...
        pc._g.append('svg:g').attr('class', 'axis').attr('transform', 'translate(0,0)').each(function (d) {
          ...
          axisElement.selectAll('line').style('fill', 'none').style('stroke', '#222').style('shape-rendering', 'crispEdges');
        }).append('svg:text').style("text-anchor", "start").style("transform", "rotate(340deg)").attr('y', -10).attr('transform', 'translate(0,-5) rotate(' + config.dimensionTitleRotation + ')').attr('x', 0).attr('class', 'label').text(dimensionLabels(config)).on('wheel', rotateLabels(config, pc));
    ...
    function axis(orient, scale) {
      ...
      function axis(context) {
        ...
        text = text.merge(tickEnter.append("text").attr("fill", "#000").attr(x, k * spacing).attr("dy", orient === top ? "0em" : orient === bottom ? "0.71em" : "0.32em")).attr("style", "transform: rotate(340deg)");
        ...
        // 0.999 here fixes blurry rotated text on non-hidpi monitors
        tick.attr("opacity", 0.999).attr("transform", function (d) {
    ...
    var reorderable = function reorderable(config, pc, xscale, position, dragging, flags) {
      return function () {
        ...
        g.style('cursor', 'move').call(drag().on('start', function (d) {
          ...
          // hide lines while resizing
          // foreground.attr('opacity', 0);
          // background.attr('opacity', 0);
        }).on('drag', function (d) {
          dragging[d] = Math.min(w(config) + 40, Math.max(-40, this.__origin__ += event.dx));
          pc.sortDimensions();
    ...
    var singlePath = function singlePath(config, position, d, ctx) {
      ...
      Object.keys(config.dimensions).map(function (p) {
        // console.log('config.dimensions p', p, position(p), d[p], d[p] === 0.0, typeof d[p], config.dimensions[p].yscale(0.0), config.dimensions[p].yscale(d[p]));
        return [position(p), d[p] === undefined ? getNullPosition(config) : config.dimensions[p].yscale(d[p])];
      }).sort(function (a, b) {
        return a[0] - b[0];
      }).forEach(function (p, i) {
        // console.log('p i', p, i) // if entire column is 0, p[1] is undefined
    ...
    var detectDimensionTypes = function detectDimensionTypes(data) {
      return Object.keys(data[0]).reduce(function (acc, cur) {
        ...
        // cycle through each item in the column to check the type. If we find an item
        // that is not a number then we treat the entire column as that type, i.e. as a string.
        var item = 'number';
        for (const element of data) {
          item = toTypeCoerceNumbers(element[cur]);
          if(item != 'number'){
            acc[key] = item;
            return acc;
          }

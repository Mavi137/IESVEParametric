...
def encode_multipart_formdata(fields, files):
    ...
    if fields is not None:
        for key, value in fields.items():
            ...
            L.append(
                bytes('Content-Disposition: form-data; name="%s"' % key, "ASCII"))
            L.append(b'')
    ...
    if files is not None:
        ...
        for key, filename in files.items():
            L.append(bytes('--' + BOUNDARY_STR, "ASCII"))
            L.append(bytes(
                'Content-Disposition: form-data; name="%s"; filename="%s"' % (key, filename), "ASCII"))
...
# We use same timeout as in VE Cloud Sync (TIMEOUT in VEScan.cpp)
request_timeout = 300
...
class Api(object):
    ...
    def root(self):
        ...
        root = ApiObject(self, json.loads(res.read().decode(
...
class ApiObject(object):
    ...
    def _href(self, relation):
        ...
        if link is None:
            raise ValueError(
                'api object has no relation `' + relation + '`')
        return link['href']
    ...
    def refresh(self):
        ...
        link = self._links.get('self', None) or self._links.get('details', None)

        if link is None:
            raise ValueError(
                'api object has neither self nor details relation so cannot be refreshed')
        if 'templated' in link:
            raise ValueError(
                'api object has templated refresh relation so cannot be refreshed')
        update = self._api.get_json(link['href'])
        self._update(update)
        return self
    ...
    def update(self):
        ...
        if link is None:
            raise ValueError(
                'api object has no update relation so cannot be updated')
...
def open_token(path=None):
    with open(path or 'scan.token', 'r') as f:
        ...
        if line:
            ...
            try:
                building = next(item.refresh(
                ) for item in root.Buildings if item._links['details']['href'] == building_url)
            except:
                pass
